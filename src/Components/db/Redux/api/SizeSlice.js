...
export const getSizeTable = createAsyncThunk("getSizeTable", async () => {
  const response = await AxiosInstance.get(`/size/all`);
...
const sizeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // get
      .addCase(getSizeTable.pending, (state) => {
        state.status = "loading...";
      })
      .addCase(getSizeTable.fulfilled, (state, action) => {
        state.status = "succeeded";
        state.meta = action.payload.meta;
        state.data = action.payload;
      })
      .addCase(getSizeTable.rejected, (state, action) => {
        state.loading = false;
        state.status = "failed";
